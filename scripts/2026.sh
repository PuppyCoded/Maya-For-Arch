...
function rpmToDeb2026()
{
    ...
    alien -d -c -v Maya/Packages/$(ls Maya/Packages/ | grep Maya2026_64 | grep .rpm)
...
# the current directory
function installPkg2026()
{
    pacman -U --noconfirm $(ls | grep maya2026 | grep .zst)
...
function installLib2026()
{
    # Create Maya's lib folder
    mkdir -p /usr/autodesk/maya2026/lib

    # Copy libraries to Maya's lib folder
    cp -r $1/lib/libtiff/4.4.0-1/* /usr/autodesk/maya2026/lib/
    cp -r $1/lib/libjbig/2.1-23/* /usr/autodesk/maya2026/lib/
    cp -r $1/lib/libffi/3.1-3.1-32/* /usr/autodesk/maya2026/lib/

    # Link libraries to Maya's lib folder
    ln -s /usr/lib/libcrypt.so.2.0.0 /usr/autodesk/maya2026/lib/libcrypt.so.1

    # Create libmd.so into lib folder
    touch /usr/autodesk/maya2026/lib/libmd.so
}

# Delete all deb packages
# from the current directory
function rmDebPkg2026()
{
    rm -f $(ls | grep substance | grep .deb)
    rm -f $(ls | grep bifrost | grep .deb)
    rm -f $(ls | grep maya2026 | grep .deb)
    rm -f $(ls | grep mayausd | grep .deb)
    rm -f $(ls | grep pymel | grep .deb)
}

# Uninstall packages
function uninstallPkg2026()
{
    ...
    pacman -Rns --noconfirm $(pacman -Qm | grep maya2026 | awk '{print $1}')
}
